...
public class InternalAdminController {
    ...
    public InternalAdminController(BookRepo bookRepo, AuthorRepo authorRepo, PublisherRepo publisherRepo, StoreRepo storeRepo,
                                   StoreBookRepo storeBookRepo, OrderRepo orderRepo, UserRepo userRepo, OrderItemRepo orderItemRepo) {
    ...
    public Map<String, Object> analytics() {
        BigDecimal revenue = orderRepo.findAll().stream().map(Order::getTotalAmount).filter(v -> v != null).reduce(BigDecimal.ZERO, BigDecimal::add);
        return Map.of(
    ...
    public Map<String, Object> updateStatus(@PathVariable Integer orderId, @RequestBody Map<String, Object> request) {
        ...
        order.setOrderStatus(String.valueOf(request.getOrDefault("status", "PROCESSING")));
